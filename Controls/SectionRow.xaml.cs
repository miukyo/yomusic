...
namespace yomusic.Controls
{
    public sealed partial class SectionRow : UserControl
    {
        public SectionRow()
        {
            ...
            ContentScrollViewer.SizeChanged += (s, e) => UpdateScrollButtons();
        }
        ...
        private void OnDataContextChanged(FrameworkElement sender, DataContextChangedEventArgs args)
        {
            if (DataContext is SectionModel section)
            {
                ...
                foreach (var item in section.Contents)
                {
                    ...
                    card.Tapped += (s, e) => NavigateToItem(item);
                    ContentPanel.Children.Add(card);
                }
                _ = Dispatcher.RunAsync(Windows.UI.Core.CoreDispatcherPriority.Normal, () => UpdateScrollButtons());
            }
        }
        ...
        private void OnLeftClick(object sender, RoutedEventArgs e)
        {
            ...
            var newOffset = Math.Max(0, sv.HorizontalOffset - 540);
        ...
        {
            var sv = ContentScrollViewer;
            var newOffset = Math.Min(sv.ScrollableWidth, sv.HorizontalOffset + 540);
            sv.ChangeView(newOffset, null, null);
        }
        ...
        private void ReleaseCardImages()
        {
            foreach (var child in ContentPanel.Children)
            {
                if (child is Border border)
                    ReleaseImageSource(border);
            }
        }

        private static void ReleaseImageSource(DependencyObject element)
        {
            if (element is Image image && image.Source != null)
            {
                image.Source = null;
                return;
            }
            for (int i = 0; i < VisualTreeHelper.GetChildrenCount(element); i++)
                ReleaseImageSource(VisualTreeHelper.GetChild(element, i));
        }

        private void UpdateScrollButtons()
        {
            var sv = ContentScrollViewer;
            var rVisible = sv.HorizontalOffset < sv.ScrollableWidth || sv.ScrollableWidth > 0;
            RightButton.IsEnabled = sv.HorizontalOffset < sv.ScrollableWidth;
            RightButton.Visibility = rVisible ? Visibility.Visible : Visibility.Collapsed;
            LeftButton.IsEnabled = sv.HorizontalOffset > 0;
            LeftButton.Visibility = rVisible ? Visibility.Visible : Visibility.Collapsed;
        }
        ...
        private static StackPanel BuildTextStack(ContentItem item, bool center = false)
        {
            ...
            if (!string.IsNullOrEmpty(item.ArtistName) && !center)
            {
                stack.Children.Add(new TextBlock
                {
